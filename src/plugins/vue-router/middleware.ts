import type { NavigationGuardNext, RouteLocationNormalized, RouteRecord } from 'vue-router';
...
class VueRouteMiddleware {
  ...
  getMiddleware(middleware: string | Function) {
    if (this._isString(middleware)) {
      const _middleware = this.middlewares[middleware as string];
      if (_middleware && this._isFunction(_middleware)) {
